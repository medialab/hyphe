...
cp config-text_indexation.env.example config-text_indexation.env
```
...
Hyphe's internal settings are adjustable within `config-backend.env`, `config-frontend.env` and `config-text_indexation.env`. Adjust the settings values to your needs following [recommendations from the config documentation](doc/config.md).
...
If you want to use Hyphe with the text indexation feature, you need to run the following command :
