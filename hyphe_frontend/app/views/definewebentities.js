      ...
      if(list){
        ...
        list = list.map(function(obj){
          for (var i in $scope.available_archives) {
            for (var prefix of [$scope.available_archives[i].url_prefix || ""].concat($scope.available_archives[i].extra_prefixes || [])) {
              var httpVariant = prefix.replace(/^https/, "http");
              if (prefix && (obj.url.indexOf(prefix) == 0 || obj.url.indexOf(httpVariant) == 0)) {
                obj.webarchives = {
